...
class TestFailureDetection(unittest.TestCase):
    ...
    def test_failure_detection_basic(self):
        ...
        dead_node = [n for n in nodes if n["id"] == "dead_node"][0]
        self.assertEqual(dead_node["status"], "alive")
        self.assertEqual(dead_node["failures"], 0)
        ...
        self.assertGreaterEqual(dead_node["failures"], 2)
    ...
    def test_failure_threshold(self):
        ...
        nodes = provider.get_nodes()
        test_node = [n for n in nodes if n["id"] == "test_node2"][0]
        self.assertEqual(test_node["status"], "alive")
        self.assertLess(test_node["failures"], 5)
    ...
    def test_self_node_always_alive(self):
        ...
        nodes = provider.get_nodes()
        self_node = [n for n in nodes if n["id"] == "self_node"][0]
        self.assertEqual(self_node["status"], "alive")
        self.assertEqual(self_node["failures"], 0)
